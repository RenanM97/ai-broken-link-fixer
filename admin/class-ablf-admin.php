...
class ABLF_Admin {
	...
	public static function maybe_export_csv() {
		// The page/action checks below are request-shape gates that must run before
		// nonce verification — otherwise this handler would fire on every admin page.
		// Capability + nonce are both enforced before any sensitive operation occurs.
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( ! isset( $_GET['page'], $_GET['action'], $_GET['nonce'] )
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			|| 'ablf-log' !== $_GET['page']
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			|| 'export' !== $_GET['action']
		...
		// CSV escaping (RFC 4180) is performed inside csv_row() — quoting and
		// inner-quote doubling. esc_html() / esc_attr() would actively corrupt the
		// output because this is CSV, not HTML, so the standard WP escaping
		// functions don't apply here.
		// phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped
		echo self::csv_row( array( 'Date Fixed', 'Fixed By', 'Source Page', 'Original URL', 'Replacement URL', 'Redirect Created' ) );
	...
	/**
	 * Format a single CSV row per RFC 4180 (comma-separated, quoted when necessary,
	 * inner double-quotes doubled). Used by the fix-log export instead of fputcsv()
	 * so we never touch the filesystem layer that WP_Filesystem governs.
	 *
	 * @param array $fields Cell values for one row.
	 * @return string The encoded row terminated by CRLF.
	 */
	...
	public static function ajax_bulk_action() {
		...
		if ( 'ignore' === $action ) {
			...
			foreach ( $ids as $id ) {
				...
				$host   = strtolower( (string) wp_parse_url( $link->broken_url, PHP_URL_HOST ) );
