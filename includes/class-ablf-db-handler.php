<?php
/**
 * Direct database access layer for the plugin's seven custom tables
 * (ablf_broken_links, ablf_suggestions, ablf_fix_log, ablf_redirects,
 * ablf_scan_queue, ablf_allowlist, ablf_usage).
 *
 * The following Plugin Check sniffs are deliberately disabled file-wide because
 * they all flag the same expected pattern for a custom-table plugin:
 *
 *   - DirectDatabaseQuery / NoCaching: $wpdb is the correct WP API for custom
 *     tables; there is no higher-level WP function that operates on them.
 *     Per-row caching would add complexity with no benefit for the admin-only,
 *     low-volume queries used here.
 *   - PreparedSQL.InterpolatedNotPrepared / UnescapedDBParameter: every {$table}
 *     value is built by self::table(), which returns $wpdb->prefix . 'ablf_*'.
 *     $wpdb->prefix is set during WP bootstrap from wp-config.php and can never
 *     be user-controlled, so the interpolation is safe.
 *
 * All user-supplied values are still bound through $wpdb->prepare() with %d/%s
 * placeholders, and per-call phpcs:ignore comments above mark any spot where
...
class ABLF_DB_Handler {
	...
	public static function purge_excluded_broken_links( $excluded_urls, $excluded_domains, $allowlist = array() ) {
		...
		// Cast every ID to a non-negative integer via absint() so the IN() list is
		// guaranteed to contain only safe integer literals. The resulting list is then
		// inlined directly into the SQL — this is the same pattern WordPress core uses
		// when binding a dynamic-length list of integer IDs (see e.g. /wp-includes/
		// class-wp-list-util.php and wp_delete_post()).
		$ids = array_filter( array_map( 'absint', $to_delete ) );
		if ( empty( $ids ) ) {
			return 0;
		}
		$id_list = implode( ',', $ids );
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $id_list is a comma-joined list of absint()-cast integers; no prepare() placeholders are needed or applicable.
